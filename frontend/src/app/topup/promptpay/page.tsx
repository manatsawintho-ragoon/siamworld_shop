          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-foreground leading-none flex items-center gap-2">
                <i className="fas fa-qrcode text-[#003b80]"></i> เติมเงินผ่าน PromptPay
              </h1>
            ...
            {ppEnabled && step === 'amount' && (
              <motion.div key="amount" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                ...
                <div className="flex items-center gap-3 border-b border-green-100 pb-4">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-[#003b80] text-white"><i className="fas fa-qrcode text-lg"></i></div>
                  <div>
                  ...
                  <div className="bg-orange-50 border border-orange-200 rounded-xl p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-col items-center px-3">
                        <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center shadow-sm"><i className="fas fa-times text-white text-xs" /></div>
            ...
            {step === 'qr' && (
              <motion.div key="qr" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }}
                ...
                <div className={`relative p-4 bg-surface border-2 rounded-2xl shadow-sm transition-all ${qrExpired ? 'border-red-200 opacity-50' : 'border-green-100'}`}>
                  ...
                  {qrExpired && (
                    <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-white/80">
                      <div className="text-center"><i className="fas fa-clock text-red-400 text-3xl mb-2" /><p className="text-red-600 font-black text-sm">QR หมดอายุ</p></div>
                    </div>
                  )}
                </div>
                ...
                <div className="text-center space-y-0.5 bg-blue-50 py-2.5 px-8 rounded-lg border border-blue-100">
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest flex items-center justify-center gap-1"><i className="fas fa-lock text-[9px]" /> ยอดที่ต้องโอน (ถูกล็อค)</p>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2.5 max-w-sm">
                  ...
                  <p className="text-[10px] font-bold text-amber-800 leading-relaxed">แอปธนาคารจะกรอกยอด <b>฿{qrAmount.toLocaleString()}</b> ให้อัตโนมัติและ<b>ไม่สามารถเปลี่ยนแปลงได้</b> เมื่อโอนสำเร็จกดปุ่มด้านล่างเพื่ออัปโหลดสลิป</p>
                </div>
                ...
                {qrExpired ? (
                  ...
                  <button onClick={() => setStep('upload')} className="btn w-full py-4 bg-[#003b80] text-white font-black text-sm shadow-[0_4px_0_#002147] hover:shadow-[0_2px_0_#002147] hover:translate-y-[2px] active:shadow-none active:translate-y-[4px] flex items-center justify-center gap-2">
                    <i className="fas fa-file-invoice-dollar text-xs"></i> โอนเงินแล้ว (แจ้งสลิป)
                  </button>
                )}
              </motion.div>
            )}
                ...
                <div className="space-y-4">
                  ...
                  {slipPreview ? (
                    <div className="relative group rounded-xl overflow-hidden border-2 border-green-100 bg-surface-hover p-3">
                      ...
                      <button onClick={() => { setSlipFile(null); setSlipPreview(''); }} className="absolute top-2 right-2 w-8 h-8 rounded-lg bg-surface text-red-500 shadow-md flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"><i className="fas fa-times text-xs"></i></button>
                    </div>
                  ) : (
                    <button onClick={() => fileRef.current?.click()} className="w-full py-10 border-2 border-dashed border-green-200 rounded-2xl bg-surface-hover flex flex-col items-center justify-center gap-3 group hover:border-primary transition-all">
                      <div className="w-12 h-12 rounded-lg bg-surface border border-green-100 flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform"><i className="fas fa-cloud-upload-alt text-xl text-primary"></i></div>
                      <div className="text-center">
                  ...
                  <div className="p-3.5 rounded-xl border border-green-100 bg-green-50/40">
                    <label className="text-[11px] font-black text-foreground-muted uppercase tracking-widest mb-2 flex items-center gap-1.5"><i className="fas fa-tag text-primary text-[10px]" /> โค้ดส่วนลด (ถ้ามี)</label>
                    {discountInfo ? (
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button type="button" onClick={() => { setDiscountInfo(null); setDiscountCode(''); setDiscountError(''); }} className="text-[11px] font-bold text-red-500 hover:underline">ลบ</button>
                      </div>
                    ) : (
                      <div className="flex gap-2">
                        <input type="text" value={discountCode} onChange={(e) => { setDiscountCode(e.target.value); setDiscountError(''); }}
                          placeholder="เช่น WELCOME10" className="flex-1 px-3 py-2 rounded-lg border border-green-200 text-sm focus:outline-none focus:border-primary" disabled={discountChecking} />
