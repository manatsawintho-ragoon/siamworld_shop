...
export default function PromptPaySettingsPage() {
  ...
  // PromptPay-only bonus multiplier
  ...
  const handleSaveBonus = async () => {
    ...
    try {
      await api('/admin/settings', {
        method: 'PUT', token: getToken()!,
        body: { settings: [
          { key: 'topup_bonus_promptpay_enabled',    value: String(bonusEnabled) },
          { key: 'topup_bonus_promptpay_multiplier', value: String(mult) },
        ]},
      });
      adminAlert({ type: 'success', title: 'บันทึกข้อมูลแล้ว' });
    } catch { adminAlert({ type: 'error', title: 'บันทึกไม่สำเร็จ' }); }
  ...
  return (
    <div className="space-y-4 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2">
            <i className="fas fa-qrcode text-[#16a34a]" /> PromptPay / QR Code
          </h1>
          <p className="text-xs text-gray-400 mt-0.5">ตั้งค่าบัญชีพร้อมเพย์ การตรวจสลิป และโปรโมชั่นเฉพาะ PromptPay</p>
        </div>
        {!ppEnabled && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200 flex-shrink-0">
            ...
            <span className="text-xs font-bold text-amber-700">PromptPay ปิดรับชำระเงิน</span>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_0_#c5cad3,0_2px_24px_rgba(0,0,0,0.10)] border border-gray-200/70 overflow-hidden flex flex-col">
          ...
          <div className="p-5 flex-1 flex flex-col gap-3.5">
              ...
              <div className="flex items-center gap-3 pl-1">
                ...
                <div>
                  <p className={`text-sm font-black ${ppEnabled ? 'text-green-800' : 'text-red-800'}`}>
                    {ppEnabled ? 'เปิดรับชำระเงิน (PromptPay)' : 'ปิดรับชำระเงิน (PromptPay)'}
                  </p>
                  <p className={`text-[11px] font-medium mt-0.5 ${ppEnabled ? 'text-green-600' : 'text-red-500'}`}>
                    {ppEnabled ? 'ผู้เล่นสามารถเติมผ่าน PromptPay ได้' : 'ผู้เล่นเติมผ่าน PromptPay ไม่ได้'}
                  </p>
                </div>
              </div>
            ...
            <p className="text-[10px] text-gray-400 -mt-1.5 px-1">
              ...
              เปิด/ปิดแล้วต้องกด &quot;บันทึกข้อมูล&quot; ฝั่งซ้ายเพื่อให้มีผลที่หน้าเติมเงิน
            </p>
      ...
      {/* ── PromptPay Bonus Multiplier Card ──────────────────────────────── */}
      <div className="bg-white rounded-2xl shadow-[0_4px_0_#c5cad3,0_2px_24px_rgba(0,0,0,0.10)] border border-gray-200/70 overflow-hidden">
        ...
        <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50/60 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-gray-900 text-sm">โปรโมชั่นเติมเงิน (เฉพาะ PromptPay)</h3>
              <p className="text-[11px] text-gray-400">ตั้งค่าโบนัสคูณยอดสำหรับการเติมผ่าน PromptPay</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
              ...
              <p className="text-[10px] font-bold text-amber-800 leading-relaxed">
                โบนัสนี้ใช้กับการเติมผ่าน PromptPay เท่านั้น
