...
const RARITY_WEIGHTS: Record<string, { default: number; hint: string }> = {
  common:    { default: 500, hint: '~50%   ไอเทมทั่วไป'  },
  uncommon:  { default: 300, hint: '~30%   ไอเทมดี'      },
...
const emptyBox  = (): Partial<LootBox>     => ({ name: '', description: '', image: '', price: 0, active: true, sort_order: 0 });
          ...
          >
            ...
            <div className="absolute inset-0 bg-gray-900/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 px-3">
              <button onClick={e => { e.stopPropagation(); onEdit(item); }}
                ...
                <i className="fas fa-pen text-[9px]" /> แก้ไขไอเทม
              </button>
...
export default function AdminLootBoxes() {
  ...
  const handleSaveBox = async () => {
    ...
    try {
      const body = { name: boxModal.name, description: boxModal.description || null, image: boxModal.image || null, price: Number(boxModal.price), sort_order: Number(boxModal.sort_order) || 0, active: boxModal.active !== false };
  ...
  const handleDeleteBox = async (id: number) => {
    ...
    try { await api(`/admin/lootboxes/${id}`, { method: 'DELETE', token: TOKEN() }); if (activeBox?.id === id) setActiveBox(null); load(); }
    catch (err: any) { alert(err?.message || 'ลบไม่สำเร็จ'); }
  };
  ...
  const handleSaveItem = async () => {
    ...
    if (!itemModal.name?.trim())    { setItemError('กรุณาใส่ชื่อไอเทม'); return; }
  ...
  const handleDeleteItem = async (itemId: number) => {
    ...
    try { await api(`/admin/lootboxes/items/${itemId}`, { method: 'DELETE', token: TOKEN() }); load(); }
    catch (err: any) { alert(err?.message || 'ลบไม่สำเร็จ'); }
  };
  ...
  return (
    <div className="flex flex-col gap-4" style={{ height: 'calc(100vh - 136px)' }}>
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="hidden md:flex items-center gap-2">
          {[
            ...
            { v: boxes.reduce((s, b) => s + b.items.length, 0),   label: 'ไอเทมรวม',    icon: 'fa-cubes',      color: 'text-blue-500',   bg: 'bg-blue-50'   },
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        {/* ── Left: Box List ─────────────────────────────────────────── */}
        <div className="w-[280px] flex-shrink-0 flex flex-col rounded-2xl shadow-[0_4px_0_#c5cad3,0_2px_24px_rgba(0,0,0,0.10)] border border-gray-200/70 bg-white overflow-hidden">
          ...
          <div className="px-4 py-3.5 border-b border-gray-100 bg-gray-50/60 flex-shrink-0">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-400">{boxes.length} กล่อง</p>
              </div>
              <button
            ...
            {filteredBoxes.map(box => {
              ...
              return (
                <div
                  ...
                  className={`rounded-xl cursor-pointer transition-all group border-2 ${
                    isSelected
                      ? isOpen
                ...
                >
                  <div className="flex items-center gap-2.5 px-3 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {!isOpen && (
                          <span className="text-[8px] px-1.5 py-0.5 rounded-md bg-gray-300 text-gray-500 font-bold flex-shrink-0 uppercase tracking-wide">
                            ปิด
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className={`text-[11px] font-bold ${isOpen ? 'text-gray-600' : 'text-gray-400'}`}>฿{parseFloat(String(box.price)).toLocaleString()}</span>
                        <span className={`text-[10px] ${isOpen ? 'text-gray-300' : 'text-gray-300'}`}>·</span>
                        <span className={`text-[11px] ${isOpen ? 'text-gray-400' : 'text-gray-400'}`}>{box.items.length} ไอเทม</span>
                      </div>
                    </div>
                    {/* Hover actions */}
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onClick={e => e.stopPropagation()}>
                      <button onClick={() => { setBoxError(''); setBoxModal({ ...box }); }}
                        className="w-6 h-6 rounded-md bg-amber-500 flex items-center justify-center text-white text-[9px] shadow-[0_2px_0_#b45309] hover:brightness-110 active:translate-y-px transition-all">
                        <i className="fas fa-pen" />
                      </button>
                      <button onClick={() => handleToggleActive(box)}
                        className={`w-6 h-6 rounded-md flex items-center justify-center text-white text-[9px] active:translate-y-px transition-all hover:brightness-110 ${box.active ? 'bg-[#16a34a] shadow-[0_2px_0_#0d6b2e]' : 'bg-gray-400 shadow-[0_2px_0_#9ca3af]'}`}>
                        <i className={`fas ${box.active ? 'fa-eye' : 'fa-eye-slash'}`} />
                      </button>
                      <button onClick={() => handleDeleteBox(box.id)}
                        className="w-6 h-6 rounded-md bg-red-500 flex items-center justify-center text-white text-[9px] shadow-[0_2px_0_#b91c1c] hover:brightness-110 active:translate-y-px transition-all">
                        <i className="fas fa-trash" />
                      </button>
                    </div>
                  </div>
                  ...
                  {box.items.length > 0 && (
                    <div className="mx-3 mb-2.5">
                      <div className={`flex h-1.5 rounded-full overflow-hidden gap-px ${isOpen ? 'bg-gray-100' : 'bg-gray-200'}`}>
                        {RARITY_ORDER.map(r => {
          ...
          {!activeBox ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 select-none">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-300 mt-1">เพื่อจัดการไอเทมภายในกล่อง</p>
              </div>
            </div>
          ) : (
            <>
              {/* ── Panel header ── */}
              <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50/60 flex items-center gap-3 flex-shrink-0">
                <div className="w-9 h-9 rounded-xl border border-gray-200 bg-white flex items-center justify-center flex-shrink-0 overflow-hidden shadow-[0_1px_0_#e5e7eb]">
                  {activeBox.image ? <img src={activeBox.image} alt="" className="w-full h-full object-cover" /> : <i className="fas fa-box text-gray-300 text-sm" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-gray-900 text-sm truncate">{activeBox.name}</h3>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold flex-shrink-0 ${activeBox.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {activeBox.active ? 'เปิดขาย' : 'ปิด'}
                    </span>
                  </div>
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    ฿{parseFloat(String(activeBox.price)).toLocaleString()} · {activeBox.items.length} ไอเทม
                  </p>
                </div>
                {/* Rarity count summary */}
                <div className="hidden xl:flex items-center gap-1.5 mr-2">
                  {RARITY_ORDER.map(r => {
                    const count = activeBox.items.filter(i => i.rarity === r).length;
                    if (!count) return null;
                    const rc = RARITY_CONFIG[r];
                    return (
                      <div key={r} className="flex items-center gap-1 px-2 py-1 rounded-lg border-2 text-white"
                        style={{ backgroundColor: rc.color, borderColor: rc.color + 'cc', boxShadow: `0 2px 0 ${rc.color}66` }}>
                        <span className="text-[13px] font-black leading-none tabular-nums">{count}</span>
                        <span className="text-[9px] font-bold leading-none opacity-80">{rc.label}</span>
                      </div>
                    );
                  })}
                </div>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-2 bg-[#16a34a] text-white text-xs font-bold rounded-lg shadow-[0_4px_0_#0d6b2e] hover:brightness-110 transition-all active:shadow-[0_1px_0_#0d6b2e] active:translate-y-px flex-shrink-0"
                >
                  <i className="fas fa-plus text-[10px]" /> เพิ่มไอเทม
                </button>
              </div>

              {/* ── Toolbar ── */}
              <div className="px-5 py-2.5 border-b border-gray-100 flex-shrink-0 space-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="relative flex-1 min-w-[160px] max-w-xs">
                    ...
                    <input value={itemSearch} onChange={e => setItemSearch(e.target.value)}
                      ...
                      placeholder={`ค้นหา... (${activeBox.items.length})`}
                    />
                    {itemSearch && (
                  ...
                  <select value={sortKey} onChange={e => setSortKey(e.target.value as SortKey)}
                    className="text-xs px-2.5 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:border-[#637469] text-gray-600 bg-gray-50">
                  ...
                  <div className="flex rounded-lg border border-gray-200 overflow-hidden flex-shrink-0">
                    <button onClick={() => setViewMode('grid')}
                      className={`px-2.5 py-1.5 text-xs transition-colors ${viewMode === 'grid' ? 'bg-[#1e2735] text-white' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}>
                      <i className="fas fa-grid-2" />
                    </button>
                    <button onClick={() => setViewMode('list')}
                      className={`px-2.5 py-1.5 text-xs border-l border-gray-200 transition-colors ${viewMode === 'list' ? 'bg-[#1e2735] text-white' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}>
                      <i className="fas fa-list" />
                    </button>
                  </div>
                  <button onClick={() => setGroupByRarity(!groupByRarity)}
                    className={`flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-lg border transition-all flex-shrink-0 ${groupByRarity ? 'bg-[#1e2735] text-white border-[#1e2735]' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}>
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-16 text-gray-300 gap-3">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-semibold text-gray-400">
                        {activeBox.items.length === 0 ? 'ยังไม่มีไอเทม' : 'ไม่พบไอเทมที่ค้นหา'}
                      </p>
                      <p className="text-xs text-gray-300 mt-1">
                        {activeBox.items.length === 0 ? 'กด "เพิ่มไอเทม" เพื่อเริ่ม' : 'ลองเปลี่ยนเงื่อนไขการค้นหา'}
                      </p>
                    </div>
                  </div>
                ) : groupByRarity ? (
      ...
      {boxModal && typeof window !== 'undefined' && createPortal(
        <BoxModal box={boxModal} saving={boxSaving} error={boxError} onChange={setBoxModal} onSave={handleSaveBox} onClose={() => { if (!boxSaving) setBoxModal(null); }} />,
        document.body
      )}
...
function BoxModal({ box, saving, error, onChange, onSave, onClose }: {
  box: Partial<LootBox>; saving: boolean; error: string;
      ...
      <div className="bg-white rounded-2xl shadow-[0_4px_0_#c5cad3,0_8px_40px_rgba(0,0,0,0.2)] w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1.5">ลำดับแสดง</label>
              <div className="relative">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300"><i className="fas fa-sort text-sm" /></div>
                <input type="number" value={box.sort_order ?? 0} onChange={e => onChange({ ...box, sort_order: Number(e.target.value) })}
                  className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#637469] focus:ring-2 focus:ring-[#637469]/20 transition-colors" />
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-[0_4px_0_#c5cad3,0_8px_40px_rgba(0,0,0,0.2)] w-full max-w-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/60 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-gray-900">{item.id ? 'แก้ไขไอเทม' : 'เพิ่มไอเทมใหม่'}</h3>
        ...
        <div className="px-6 py-5">
          ...
          <div className="mb-5">
            <label className="block text-xs font-bold text-gray-500 mb-1.5">ชื่อไอเทม <span className="text-red-400">*</span></label>
            <div className="relative">
