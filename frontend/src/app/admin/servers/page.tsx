...
function AuthMeGuide() {
  const [open, setOpen] = useState(false);

  const configLines = [
    'DataSource:',
    '  backend: MYSQL',
    '  caching: false          # ต้อง false เพราะมี web integration',
    '  mySQLHost: <ดูใน deploy credentials>',
    '  mySQLPort: \'<MySQL exposed port>\'',
    '  mySQLUseSSL: true',
    '  mySQLCheckServerCertificate: false',
    '  mySQLAllowPublicKeyRetrieval: true',
    '  mySQLUsername: siamworld',
    '  mySQLPassword: \'<password จาก .env>\'',
    '  mySQLDatabase: siamworld',
    '  mySQLTablename: authme',
    '  poolSize: 10',
    '  maxLifetime: 1770       # ต้อง < 1800 (MySQL wait_timeout)',
  ];

  return (
    <div className="bg-white rounded-2xl shadow-[0_4px_0_#c5cad3,0_2px_16px_rgba(0,0,0,0.08)] border border-gray-200/70 overflow-hidden">
      <button
        onClick={() => setOpen(v => !v)}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[13px] font-black text-gray-900">คู่มือ AuthMe Plugin (config.yml)</p>
            <p className="text-[11px] text-gray-400 mt-0.5">ตั้งค่า AuthMe ให้เชื่อมต่อ MySQL ของ SiamSite ได้ถูกต้อง — แก้ปัญหาผู้เล่นถูกขอ register ซ้ำ</p>
          </div>
        </div>
        <i className={`fas fa-chevron-down text-gray-400 text-[11px] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
        ...
        <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-red-50 border border-red-200">
              <i className="fas fa-exclamation-triangle text-red-500 text-sm mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-[12px] font-black text-red-700">caching: false (บังคับ)</p>
                <p className="text-[11px] text-red-600 mt-0.5 leading-relaxed">
                  AuthMe ห้ามเปิด caching เมื่อมี website integration เพราะ cache จะ stale ทำให้ผู้เล่นเก่าถูกขอ register ใหม่
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200">
              <i className="fas fa-clock text-amber-500 text-sm mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-[12px] font-black text-amber-700">maxLifetime: 1770 (บังคับ)</p>
                <p className="text-[11px] text-amber-600 mt-0.5 leading-relaxed">
                  ต้องน้อยกว่า MySQL <code className="font-mono bg-amber-100 px-1 rounded">wait_timeout</code> (1800) อย่างน้อย 30 วินาที — ถ้าสูงกว่าจะเกิด stale connection
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Config template */}
          <div>
            <p className="text-[11px] font-black text-gray-500 uppercase tracking-wide mb-2">แก้ไข plugins/AuthMe/config.yml</p>
            <div className="bg-[#1e2735] rounded-xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5">
                <i className="fas fa-file-code text-gray-500 text-[11px]" />
                <span className="text-[11px] text-gray-500 font-mono">plugins/AuthMe/config.yml</span>
              </div>
              <pre className="px-4 py-3 text-[11px] font-mono text-gray-300 leading-relaxed overflow-x-auto whitespace-pre">
{configLines.map((line, i) => {
  ...
  if (isComment) {
    return (
      <span key={i}>
        <span className="text-gray-100">{line.slice(0, commentIdx)}</span>
        <span className="text-green-400">{line.slice(commentIdx)}</span>
        {'\n'}
      </span>
    );
  }
  ...
  if (colonIdx > 0 && !line.trim().startsWith('#')) {
    return (
      <span key={i}>
        <span className="text-[#79b8ff]">{line.slice(0, colonIdx + 1)}</span>
        <span className="text-[#f97316]">{line.slice(colonIdx + 1)}</span>
        {'\n'}
      </span>
    );
  }
  return <span key={i}>{line}{'\n'}</span>;
          ...
          {/* Steps */}
          <div>
            <p className="text-[11px] font-black text-gray-500 uppercase tracking-wide mb-2">ขั้นตอน</p>
            <ol className="space-y-2">
              {[
                { n: '1', text: 'ดู MySQL host, port, password ใน deploy credentials (.env ของลูกค้า) หรือถามทีม SiamSite' },
                { n: '2', text: 'แก้ config.yml ตามตัวอย่างด้านบน — เฉพาะ caching, mySQLHost, mySQLPort, mySQLPassword, maxLifetime' },
                { n: '3', text: 'Restart Minecraft server เพื่อ reload config' },
                { n: '4', text: 'ทดสอบ login — ผู้เล่นเก่าควร login ได้ทันทีโดยไม่ถูกขอ register ใหม่' },
              ].map(s => (
                <li key={s.n} className="flex items-start gap-3">
                  <span className="w-5 h-5 rounded-full bg-[#1e2735] text-white text-[10px] font-black flex items-center justify-center flex-shrink-0 mt-0.5">{s.n}</span>
                  <p className="text-[12px] text-gray-600 leading-relaxed">{s.text}</p>
                </li>
              ))}
            </ol>
          </div>
