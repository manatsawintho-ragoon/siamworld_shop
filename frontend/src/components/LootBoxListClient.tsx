  ...
  return (
    <div className={`group relative flex flex-col bg-surface border rounded-xl overflow-hidden transition-all duration-200 hover:shadow-theme-md ${
      ...
      <div className="relative aspect-[3/4] bg-accent/10 overflow-hidden">
        ...
        {isLimitedBox && (
          <span className="absolute top-1.5 left-1.5 z-10 flex items-center gap-0.5 bg-violet-600 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full shadow-sm leading-none">
            <Gem className="w-2 h-2" strokeWidth={2.5} /> LIMITED
          </span>
        )}
        {!isLimitedBox && isHotBox && (
          <span className="absolute top-1.5 left-1.5 z-10 flex items-center gap-0.5 bg-amber-500 text-amber-950 text-[10px] font-black px-1.5 py-0.5 rounded-full shadow-sm leading-none">
        ...
        <div className={`px-2.5 py-1.5 border-t text-[10px] ${isPaused ? 'bg-warning/8 border-warning/20' : soldOut ? 'bg-error/8 border-error/20' : expired ? 'bg-surface-hover border-border-muted' : 'bg-accent/10 border-accent/20'}`}>
          ...
          {(active || unlimited) && (
            <div className="flex items-center justify-between gap-1">
              ...
              {remaining !== null && box.stock_limit! > 0 && (
                <span className={`flex items-center gap-0.5 font-black tabular-nums ml-auto flex-shrink-0 ${soldOut ? 'text-error' : stockPct <= 20 ? 'text-orange-700' : 'text-foreground-muted'}`}>
          ...
          {!active && !unlimited && remaining !== null && box.stock_limit! > 0 && (
            <div className="flex items-center justify-between">
              <span className={`flex items-center gap-1 font-bold ${soldOut ? 'text-error' : stockPct <= 20 ? 'text-orange-700' : 'text-foreground-muted'}`}>
...
const TYPE_FILTERS: { key: TypeFilter; label: string; Icon: LucideIcon; color: string; activeBg: string; activeShadow: string; activeText: string; themed?: boolean }[] = [
  ...
  { key: 'limited', label: 'LIMITED',   Icon: Gem,     color: 'text-violet-600',       activeBg: 'bg-violet-600', activeShadow: 'shadow-[0_3px_0_#5b21b6]', activeText: 'text-white' },
  { key: 'hot',     label: 'ยอดนิยม',   Icon: Flame,   color: 'text-amber-700',        activeBg: 'bg-amber-500',  activeShadow: 'shadow-[0_3px_0_#b45309]', activeText: 'text-amber-950' },
...
/** `initial` comes from the server shell in app/lootbox/page.tsx - see ShopClient. */
export default function LootBoxListClient({ initial }: { initial: LootBox[] }) {
  const [boxes]                 = useState<LootBox[]>(initial);
  const loading = false;
