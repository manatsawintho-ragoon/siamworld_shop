          ...
          return (
            <Link key={box.id} href={`/lootbox/${box.id}`}
              ...
              <div className="relative aspect-[3/4] bg-surface-hover overflow-hidden">
                ...
                {box.image ? <img src={proxyImage(box.image, 256)} alt={box.name} className="w-full h-full object-contain p-4 group-hover:scale-105 transition-transform duration-300" onError={onProxyError} /> : <div className="w-full h-full flex items-center justify-center"><Package className="w-12 h-12 text-foreground-subtle/30" strokeWidth={1.5} /></div>}
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-5">
        ...
        <div className="flex flex-col gap-3 lg:w-[230px] flex-shrink-0 lg:self-start">
            ...
            <div className={CARD_CLS}>
              ...
              <div className="px-2 py-2 space-y-0.5">{recentBox.length === 0 ? <p className="text-foreground-subtle text-xs text-center py-6">ยังไม่มีข้อมูล</p> : recentBox.slice(0, 5).map((r, i) => { const rar = getRarity(r.item_rarity); return (<div key={i} className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-surface-hover transition-colors"><div className="w-10 h-10 rounded-xl flex-shrink-0 border border-border bg-surface-hover flex items-center justify-center">{r.item_image ? <img src={proxyImage(r.item_image, 36)} alt={r.item_name} className="w-9 h-9 object-contain" style={{ imageRendering: 'pixelated' }} onError={onProxyError} /> : <Gem className="w-4 h-4 text-foreground-subtle" strokeWidth={2} />}</div><div className="min-w-0 flex-1"><p className="text-foreground text-[11px] font-bold truncate leading-snug">{r.item_name}</p><p className="text-foreground-subtle text-[9px] truncate">{r.username}</p></div><div className="flex flex-col items-end gap-0.5 flex-shrink-0"><span className="inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-[0.1em] px-1.5 py-0.5 rounded-md text-white leading-none" style={{ backgroundColor: rar.color, boxShadow: `0 1px 0 ${rar.color}88` }}><span className="w-1 h-1 rounded-sm bg-white/60 flex-shrink-0" />{rar.label}</span><span className="text-foreground-subtle text-[8px] tabular-nums">{timeAgo(r.won_at)}</span></div></div>); })}</div>
            </div>
            )}
            ...
            <div className={CARD_CLS}>
              ...
              <div className="px-2 py-2 space-y-0.5">{recentBuy.length === 0 ? <p className="text-foreground-subtle text-xs text-center py-6">ยังไม่มีข้อมูล</p> : recentBuy.slice(0, 5).map((r, i) => (<div key={i} className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-surface-hover transition-colors"><div className="w-10 h-10 rounded-xl flex-shrink-0 border border-border bg-surface-hover flex items-center justify-center">{r.image ? <img src={proxyImage(r.image, 36)} alt={r.product_name} className="w-9 h-9 object-contain" style={{ imageRendering: 'pixelated' }} onError={onProxyError} /> : <Package className="w-4 h-4 text-foreground-subtle" strokeWidth={2} />}</div><div className="min-w-0 flex-1"><p className="text-foreground text-[11px] font-bold truncate leading-snug">{r.product_name}</p><p className="text-foreground-subtle text-[9px] truncate">{r.username}</p></div><div className="flex flex-col items-end gap-0.5 flex-shrink-0"><div className="inline-flex items-center gap-1 text-[8px] font-black px-1.5 py-0.5 rounded-md leading-none" style={{ backgroundColor: 'rgb(var(--color-primary))', color: 'rgb(var(--color-primary-foreground))', boxShadow: '0 1px 0 rgb(var(--color-primary-hover) / 0.53)' }}><Coins className="w-2 h-2" strokeWidth={2.5} />{parseFloat(String(r.price)).toLocaleString()}</div><span className="text-foreground-subtle text-[8px] tabular-nums">{timeAgo(r.created_at)}</span></div></div>))}</div>
            </div>)}
        </div>
      </div>
    </MainLayout>
