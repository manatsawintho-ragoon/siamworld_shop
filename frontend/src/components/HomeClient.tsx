      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-5">
        ...
        <div className="flex flex-col gap-3 lg:w-[230px] flex-shrink-0 lg:self-start">
            ...
            <div className={CARD_CLS}>
              ...
              <div className="px-2 py-2 space-y-0.5">{recentBuy.length === 0 ? <p className="text-foreground-subtle text-xs text-center py-6">ยังไม่มีข้อมูล</p> : recentBuy.slice(0, 5).map((r, i) => (<div key={i} className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-surface-hover transition-colors"><div className="w-10 h-10 rounded-xl flex-shrink-0 border border-border bg-surface-hover flex items-center justify-center">{r.image ? <img src={r.image} alt={r.product_name} className="w-9 h-9 object-contain" style={{ imageRendering: 'pixelated' }} /> : <Package className="w-4 h-4 text-foreground-subtle" strokeWidth={2} />}</div><div className="min-w-0 flex-1"><p className="text-foreground text-[11px] font-bold truncate leading-snug">{r.product_name}</p><p className="text-foreground-subtle text-[9px] truncate">{r.username}</p></div><div className="flex flex-col items-end gap-0.5 flex-shrink-0"><div className="inline-flex items-center gap-1 text-[8px] font-black px-1.5 py-0.5 rounded-md leading-none" style={{ backgroundColor: 'rgb(var(--color-primary))', color: 'rgb(var(--color-primary-foreground))', boxShadow: '0 1px 0 rgb(var(--color-primary-hover) / 0.53)' }}><Coins className="w-2 h-2" strokeWidth={2.5} />{parseFloat(String(r.price)).toLocaleString()}</div><span className="text-foreground-subtle text-[8px] tabular-nums">{timeAgo(r.created_at)}</span></div></div>))}</div>
            </div>)}
        </div>
      </div>
    </MainLayout>
